...
//MAKEUP ASSIGNMENT CODE
//add a function that will define the pop-up menu
function addMenu(win) {
	var activity = win.activity;
	
	activity.onCreateOptionsMenu = function(e) {
		var firstItem = e.menu.add({ title: 'Help'});
		firstItm.addEventListener("click", function (e) {Ti.API.debug ('Help'); });

		var secondItem = e.menu.add({title: 'Settings'});
		secondItem.addEventListener("click", function(e) {Ti.API.warn('Settings');});
		
		var thirdItem = e.menu.add({ title: 'Feedback'});
		secondItem.addEventListener("click". function(e) {alert('Feedback'); );
	};
}
addMenu(win1);
...
if (isAndroid()) {
	...
	activity.onCreateOptionsMenu = function(e) {
		...
		var firstItem = e.menu.add({ title: 'Help'});
		firstItem.addEventListener("click", function(e) {Ti.API.debug('Help'); });
